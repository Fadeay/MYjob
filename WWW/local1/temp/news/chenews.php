<?php
  ...
  if (empty($_GET["news"])) {
    ...
    $news = htmlspecialchars(mysqli_real_escape_string($con,$_GET["news"]));
  }
...
<body>
  ...
  <div class="container">
    <div class="row">
      <form action="./php/chenews.php?news=<?php echo $news ?>" enctype="multipart/form-data" method="post">
        <div class="form-group">
          <div class="input-group">
              <span class="input-group-addon">标题</span>
              <input type="text" class="form-control" placeholder="请输入新闻标题" name="title" value="<?php echo $rown[1] ?>" required autofocus/>
          </div>
        </div>
        <div class="form-group">
          正文：<textarea class="form-control" rows="20" name="news" required><?php echo $rown[2] ?></textarea>
        </div>
        <div class="form-group">
